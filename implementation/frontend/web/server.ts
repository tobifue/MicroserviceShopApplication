...
class HttpOption {
    ...
    constructor(path: string, method: string = 'POST', host: string = 'localhost', port: number = 8080, headers: { "Content-Type": string } = { "Content-Type": "application/json" }) {
        this.host = host;
        this.port = port;
        this.path = path;
        this.method = method;
...
app.get('/vendor', function (req, res, next) {
    const httpreqGetItems = http.get("http://localhost:8080/inventory/getItems/1", response => {
        let items: string = "";
